...
// Get all subtitles
router.get("/subtitles", authenticateUser, async (req: Request, res: Response) => {
    try {
...
router.get("/subtitles/:movieTitle", authenticateUser, async (req: Request, res: Response) => {
    try {
        const movieTitle = decodeURIComponent(req.params.movieTitle);
        const subtitles = await Subtitle.find({ movieTitle });

        if (subtitles.length === 0) {
            return res.status(404).json({ message: "No subtitles found for this movie" });
        }
...
router.get("/subtitle/:movieTitle/:language", authenticateUser, async (req: Request, res: Response) => {
    try {
        const movieTitle = decodeURIComponent(req.params.movieTitle);
        ...
        if (fs.existsSync(subtitle.path)) {
            res.sendFile(subtitle.path);
        } else {
            res.status(404).json({ message: "Subtitle file not found on disk" });
        }
    } catch (error: unknown) {
        if (error instanceof Error) {
            res.status(500).json({ message: error.message });
...
// Add a new subtitle
router.post("/subtitles", authenticateUser, async (req: Request, res: Response) => {
    try {
...
// Delete a subtitle
router.delete("/subtitles/:id", authenticateUser, async (req: Request, res: Response) => {
    try {
